...
from upsnet.models.efficientnet import EfficientNetBackbone
...
class resnet_upsnet(resnet_rcnn):
    ...
    def __init__(self, backbone_depth):
        ...
        # backbone net
        self.resnet_backbone = EfficientNetBackbone(backbone_depth)
    ...
    def forward(self, data, label=None):
        ...
        # [1, 256, 200, 272], [1, 512, 100, 136], [1, 1024, 50, 68], [1, 2048, 25, 34]
        # print("res2, res3, res4, res5", res2.size(), res3.size(), res4.size(), res5.size())
        fpn_p2, fpn_p3, fpn_p4, fpn_p5, fpn_p6 = self.fpn(res2, res3, res4, res5)
        # print("fpn_p2, fpn_p3, fpn_p4, fpn_p5, fpn_p6", fpn_p2.size(), fpn_p3.size(), fpn_p4.size(), fpn_p5.size(), fpn_p6.size())
        # [1, 256, 200, 272], [1, 512, 100, 136], [1, 1024, 50, 68], [1, 2048, 25, 34], [1, 256, 13, 17]
        ...
        for feat in [fpn_p2, fpn_p3, fpn_p4, fpn_p5, fpn_p6]:
            ...
            # print("rpn_cls_score_p, rpn_bbox_pred_p, rpn_cls_prob_p", rpn_cls_score_p.size(), rpn_bbox_pred_p.size(), rpn_cls_prob_p.size())
            # 和feat 的shape一致
        ...
        if label is not None and config.train.fcn_with_roi_loss:
            ...
            # print("fcn_output", fcn_output['fcn_output'].size())
            # [1, 133, 800, 1088]
        else:
